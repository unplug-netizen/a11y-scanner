...
1. **webhooks** - Speichert Webhook-Konfigurationen für Benutzer
2. **focus_scans** - Speichert Fokus-Scan-Ergebnisse
3. **false_positive_rules** - Speichert Regeln für False Positives

## Status

### Problem
- Kein direkter SQL-Zugang via psql möglich (IPv6/IPv4 Netzwerk-Inkompatibilität)
- Supabase CLI nicht mit Service Role Key nutzbar

### Lösung
Die Migration muss manuell im Supabase SQL Editor ausgeführt werden:

1. Öffne https://supabase.com/dashboard
2. Wähle das Projekt: `onxjagnofgxcmxolrxdi`
3. Gehe zu "SQL Editor"
4. Füge den Inhalt von `20250319_phase42_complete_migration.sql` ein
5. Klicke auf "Run"
...
## API-Routen
Die folgenden API-Routen sind bereits implementiert und nutzen die Tabellen:

- `/api/webhooks` - Webhook-Verwaltung
- `/api/false-positives` - False Positive Regeln
- `/api/focus-order` - Fokus-Scan Funktionalität

## Nächste Schritte
1. Migration im Supabase SQL Editor ausführen
2. Schema-Cache aktualisieren (passiert automatisch)
3. API-Routen testen
